...
data_url=http://www.openslr.org/resources/5/switchboard_word_alignments.tar.gz
...
local/swbd_data_prep.sh $data || exit 1

num_dev_sentences=10000
mkdir -p `dirname "$train_file"`
cut -d' ' -f2- $data/text | tail -n +$num_dev_sentences > $train_file
mkdir -p `dirname "$valid_file"`
cut -d' ' -f2- $data/text | head -n $num_dev_sentences > $valid_file
